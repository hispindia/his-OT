		...
		<c:forEach var="otProcedure" items="${otProcedures}" varStatus="index">
			...
			<c:choose>
				...
				<td id="acceptBox_${otProcedure.orderId}">					
					<c:choose>
						<c:when test="${empty otProcedure.status}">
							<a href='javascript:acceptProcedure(${otProcedure.orderId});'>
								Accept 
							</a>
						</c:when>
				...
				<td id="rescheduleBox_${otProcedure.orderId}">
					<c:choose>
						<c:when test="${empty otProcedure.status}">
							<a href="rescheduleProcedure.form?orderId=${otProcedure.orderId}&modal=true&height=220&width=800" class="thickbox" title="Reschedule Procedure">Reschedule</a>
						</c:when>
						<c:when test="${otProcedure.status eq 'accepted'}">
							Reschedule					
						</c:when>
